...
struct exec_info 
  {
    ...
    struct childprocess *childprocess;    /* Child process. */
...
process_execute (const char *file_name) 
{
  ...
  if (tid != TID_ERROR)
    {
      /*   parent blocked to wait for child loading
      parent thread cannot return after process_execute() 
      until load user program successfully */ 
      sema_down (&exec.load_done);
      if (exec.success)
        list_push_back (&thread_current ()->children, &exec.childprocess->elem);
      else 
        /* if the child process is not loaded  */
        tid = TID_ERROR;
    }
  else
    dir_close (exec.wd);
...
start_process (void *exec_)
{
  ...
  /* Allocate childprocess. */
  if (success)
    {
      exec->childprocess = thread_current ()->childprocess
        = malloc (sizeof *exec->childprocess);
      success = exec->childprocess != NULL; 
    }

  /* Initialize childprocess. */
  if (success) 
    {
      lock_init (&exec->childprocess->lock);
      exec->childprocess->ref_cnt = 2;
      exec->childprocess->tid = thread_current ()->tid;
      sema_init (&exec->childprocess->dead, 0);
    }
...
process_wait (tid_t child_tid) 
{
  ...
  for (e = list_begin (&cur->children); e != list_end (&cur->children);
       e = list_next (e)) 
    {
      struct childprocess *cs = list_entry (e, struct childprocess, elem);
      if (cs->tid == child_tid) 
        {
          int exit_status;
          list_remove (e);
          /* sema down to block parent thread util child process is dead */
          sema_down (&cs->dead);
          exit_status = cs->exit_status;
          
          /* update status and check if struct can be freed */
          lock_acquire (&cs->lock);
          int new = --cs->ref_cnt;
          lock_release (&cs->lock);
          /* only release when parent is dead */
          if (new == 0)
            free (cs);
...
process_exit (void)
{
  ...
  if (cur->childprocess != NULL) 
    {
      struct childprocess *cs = cur->childprocess;
      ...
      lock_release (&cs->lock);
      /* only release the struct when both are dead */
      if (new_ref_cnt == 0)
        free (cs);
    }
...
setup_stack (const char *cmd_line, void **esp) 
{
  ...
  int argc;
  char **argv;
  char *fn_copy = push (kpage, &ofs, cmd_line, strlen (cmd_line) + 1);
  if (fn_copy == NULL){
    lock_release (&(page->frame)->lock);
    return false;
  }
  
  /* According to the C convention, first put a 0,
      in case there is nno parameter */
  if (!push (kpage, &ofs, &null, sizeof null)){
    lock_release (&(page->frame)->lock);
    return false;
  }
  
  /* call strtok_r() to separate out a single argumemt (the string),
    copy each string to the user stack, 
    and record the position on the stack into an array. */
  argc = 0;
  for (karg = strtok_r (fn_copy, " ", &saveptr); karg != NULL;
       karg = strtok_r (NULL, " ", &saveptr))
    {
      void *uarg = upage + (karg - (char *) kpage);
      if (!push (kpage, &ofs, &uarg, sizeof uarg)){
        lock_release (&(page->frame)->lock);
        return false;
      }
      argc++;
  ...
  /* reverse the order of argvs */
  reverse (argc, (char **) (kpage + ofs));
  argv = (char **) (upage + ofs);

  /* push argv, argc onto stack in order */
