...
/* Creates a directory. Returns inode of created directory if successful,
            ...
            struct inode **inode) 
{
  ...
  bool suc = lookup (dir, name, &e, NULL);
...
dir_remove (struct dir *dir, const char *name) 
{
  ...
  if(inode_get_type(inode) == DIR_INODE)
  {
    ...
    /* count how many entries are using the dir */
    for(off_t scan_offset = 0; inode_read_at(inode,
        &scan_entry, sizeof scan_entry, scan_offset) 
        == sizeof scan_entry; scan_offset += sizeof scan_entry)
    {
